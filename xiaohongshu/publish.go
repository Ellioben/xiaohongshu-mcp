...
func selectFirstTagOption(contentElem *rod.Element) error {
	...
	//	"[data-testid='tag-suggestion']:first-child",
	//	".tag-list .tag-item:first-child",
	//}

	//for _, selector := range selectors {
	//	if elements, err := contentElem.Page().Elements(selector); err == nil && len(elements) > 0 {
	//		// 找到下拉框选项，点击第一个
	//		if err := elements[0].MustClick(); err == nil {
	//			slog.Info("成功选择标签选项", "selector", selector)
	//			time.Sleep(200 * time.Millisecond)
	//			return nil
	//		}
	//	}
